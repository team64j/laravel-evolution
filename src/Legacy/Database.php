...
class Database extends Manager
{
    ...
    public function query($sql, $watchError = true)
    {
        try {
            ...
            if ($watchError === true) {
                evolutionCMS()->getService('ExceptionHandler')->messageQuit(
                    e($exception->getMessage()) . '<br><br>' . e($sql)
                );
            }
        }
    }
    ...
    public function insert($fields, $intotable, $fromfields = "*", $fromtable = "", $where = "", $limit = "")
    {
        ...
        if (!$intotable) {
            evolutionCMS()->getService('ExceptionHandler')->messageQuit(
                "Empty \$intotable parameters in DBAPI::insert()."
            );
        } else {
            ...
            if (!is_array($fields)) {
                ...
                if (empty($fromtable)) {
                    ...
                    $this->query(
                        "INSERT INTO {$intotable} {$fields} SELECT {$fromfields} FROM {$fromtable} {$where} {$limit}"
                    );
                }
            }
    ...
    public function update($fields, $table, $where = "")
    {
        ...
        if (!$table) {
            evolutionCMS()->getService('ExceptionHandler')->messageQuit(
                'Empty ' . $table . ' parameter in DBAPI::update().'
            );
        } else {
